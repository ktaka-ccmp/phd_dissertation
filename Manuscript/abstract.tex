...
They also need to be able to expand their businesses to different countries, once they are proven to be successful in a country.
...
And it is expected to provide a uniform platform for container clusters\replaced{ by functioning as a middleware, which also facilitates the migration.}{ , which also facilitates the migration of web applications consisting of container clusters.}
\replaced{
However, existing container orchestrators do not function as a uniform middleware, because none of them can automatically set up the route for ingress traffic from the Internet regardless of the base infrastructures.
}{
...
In this dissertation, the author addresses this problem by \replaced{proposing}{ providing} a portable software load balancer that is runnable on any infrastructure \added{in a same manner} and capable of\replaced{ setting up the route for the ingress traffic automatically.}{ an automatic setup of the ingress traffic routing.}
\replaced{To prove the concept, the author provides}{ The author proposes} a cluster of software load balancers in container\deleted{ for Kubernetes,} that can be launched as a part of web applications\added{, for Kubernetes}.
It also supports an automatic setup of Equal Cost Multi Path(ECMP) routes to make multiple load balancers active, \replaced{in order}{ and thereby} to provide redundancy and scalability\added{ at the same time}.
By using the proposed load balancer, \deleted{ the way to route the traffic into container clusters is standardized, removing dependencies}\added{web application clusters no longer depend} on load balancer architectures provided by infrastructures.
