...
As web applications become an essential part of daily life, an outage of the web application is getting to be a bigger problem.
If something happens to a web cluster in a data center, people will not be able to access the necessary information.
...
Historically, most of the Web application providers purchased servers and installed those in server housing facilities called data centers.
...
Users only need to feed configuration files to the orchestrator to launch web applications.
...
As a result, a user can easily migrate their web applications at his or her convenience.
And migrated web applications are guaranteed to behave the same manner since container orchestrator ac as common middleware that hides difference among base infrastructures.
...
Because of the broader purpose, an out of box Mesos might not be best suited for web applications. 
...
In order to bring that into reality, the author proposes a container cluster of software load balancers.
...
The orchestrators are good at managing containers, and they can scale containers, i.e. change the number of containers depending on the needs.
...
The author investigates a container cluster of software load balancers for Kubernetes as a test case since Kubernetes seems most appropriate for web application clusters at the moment.
The author expects general findings of this investigation can be easily applied to the other container orchestrators as well.
...
In order to alleviate this problem, the author proposes a portable and scalable software load balancer that can be used in any environment including cloud providers and in on-premise data centers.
