...
Another aspect of the container orchestrator is interesting.
It can be viewed as the Operating System for a data center, which not only provides scheduling of processes but also routes the traffic to the right processes.
The orchestrator let users launch an internally dependent complex cluster of containers, with a single launch command by supplying a configuration file prepared beforehand.
By using the orchestrator, a user can start a web application that consists of a cluster of containers, as easily as starting a single process on a single computer.
