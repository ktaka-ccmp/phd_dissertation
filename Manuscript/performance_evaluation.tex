...
\added{and therefore, we can not see the real difference between ipvs performance and iptables DNAT performance.}
...
As can be seen in the figure, while the performance levels for ipvs and iptables DNAT exactly match\added{ because of the network band width limitation}, the performance level of ipvs-tun is much higher than those.
...
The throughput measurements were also carried \added{out} in GCP and AWS to show the containerized ipvs load balancer is runnable even in the cloud environment.
