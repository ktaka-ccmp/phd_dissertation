...
\added[id=2nd]{  Proposed load balancer can run both in the cloud infrastructure and in the on-premise data center and can be utilized to set up the route for ingress traffic automatically.}
...
\added{
While the method to acquire information about running containers is specific to Kubernetes, the other technologies used are not.
...
3) Performance: whether the load balancer has} \added[id=2nd]{sufficient}\deleted[id=2nd]{enough} \added{throughput for 10 Gbps network.
}
...
The author carries out throughput measurements of \replaced[id=2nd]{IPVS}{ipvs}, \replaced[id=2nd]{IPVS-TUN}{ipvs-tun}, and iptables DNAT in 10 Gbps environment.
