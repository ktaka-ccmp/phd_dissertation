...
First, the author discusses problems of convention\replaced{al}{s} architecture in Section~\ref{Problem of Conventional Architecture}.
...
Then \replaced[id=2nd]{IPVS}{ipvs} containerization is explained in detail in Section~\ref{sec:ipvs}.
...
The proxy daemon on the nodes also setup iptables DNAT \cite{MartinA.Brown2017} rules\added{ as an internal load balancer}. 
The Ingress traffic will then be evenly distributed by the cloud load balancer to all \added[id=2nd]{of} the existing nodes, 
...
\replaced{If}{Suppose} there are 1,000 nodes and one of the web applications only uses 10 nodes, distributing the ingress traffic to all \added[id=2nd]{of} the 1,000 nodes seems inefficient and complicated.
...
\begin{minipage}{0.9\columnwidth}
   The traffic from the internet is distributed by the upstream router to multiple of load balancer (lb) pods using hash-based ECMP (the solid green line), after which distributed by the lb pods to web pods using Linux kernel's \replaced[id=2nd]{IPVS}{ipvs} (the solid red line).
...
While containerizing \replaced[id=2nd]{IPVS}{ipvs} makes it runnable in any environment, it is essential to discuss how to route the ingress traffic to the \replaced[id=2nd]{IPVS}{ipvs} container.
...
The BGP and ECMP are both standard protocols supported by most of the commercial router products\added{, and they are used for cloud load balancers as well \cite{eisenbud2016maglev,patel2013ananta}}.
\added{However, in the case of container environments, where overlay networks are often used, special cares are needed to make the mechanism work properly.}
...
In addition to \deleted{that}\added{alleviate complexiety due to overlay network}, the route reflector brings another benefit.
\added{By using the route reflector, }\replaced{t}{T}he upstream router \deleted{does not}\added{no longer} need\added{s} to accept BGP sessions from containers with random IP addresses, but only from the route reflector with well known fixed IP address.
