...
For example, Google provides a variety of web  \replaced[id=5th]{application}{service}s including, Gmail, Search engines, Google Suite, etc.
...
In order to prevent such outages, preparing another web application cluster in a different location in the \replaced[id=5th]{event of a disaster}{case for disasters} is very important.
...
Due to the widespread usage of Linux systems, the Linux container can run in most of the cloud infrastructures and on-premise data centers, which is beneficial for \replaced[id=5th]{interoperability}{migrations}.
...
Container tools alleviate these problems by packing necessary libraries \added[id=5th]{for each program separately} into \added[id=5th]{respective} archives. 
...
By using Mesos user does not need to have \added[id=5th]{a} dedicated physical server cluster for each application\deleted[id=5th]{s}.
...
The author believes solving this problem is an open and important topic for research and development, and therefore intends to pursue it\added[id=5th]{ in this study}.
...
Contributions of this \replaced[id=5th]{dissertation}{thesis} can be summarized as follows:
...
In \replaced[id=5th]{Chapter~\ref{chapter:Performance Evaluation}}{the previous chapter}, the author evaluate\replaced[id=5th]{s}{d} the feasibility of the proposed load balancer in \added[id=5th]{the} 1Gbps network.
...
\added[id=5th]{And finally, }Chapter~\ref{chapter:Conclusion and future work} presents \added[id=5th]{the} conclusion of this study.
