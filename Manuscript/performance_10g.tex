\replaced[id=2nd]{In}{It} the previous chapter the proposed load balancer is verified to be portable, redundant and scalable in 1Gbps network.
...
The author carries out throughput measurements of \replaced[id=2nd]{IPVS}{ipvs}, \replaced[id=2nd]{IPVS-TUN}{ipvs-tun}, and iptables DNAT in 10Gbps environment.
The author clarifies the reasons for performance limitations of each of the results and shows that the proposed load balancer has \replaced[id=2nd]{sufficient}{enough} throughput in 10Gbps network.
...
Figure~\ref{fig:ipvs_l3dsr_10g} shows the throughput results of \replaced[id=2nd]{IPVS}{ipvs}, \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} and iptables DNAT in 10Gbps environment.
...
While iptables DNAT rules exist in node net namespace, proposed ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} exist in container net namespace.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[CPU usage of load balancers in containers]{
      ...
      The iptables DNAT rules exist in the node net namespace.
      The ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} are in containers.
...
The ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} load balancers were setup on one of the nodes. 
The load balancing rules were created in the node namespaces, and then throughput measurement were carried out.

Figure~\ref{fig:ipvs_l3dsr_10g_node_ns} shows the throughput of ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} in the node net namespace together with the throughput of the iptables DNAT.
The throughputs of the ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} are improved from the previous results.
...
On the other hand, the throughput of ipvs is still less than those of \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} and iptables DNAT.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[Throughput of load balancers in node namespace]{
      ...
      The performance levels of the ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} are greatly improved from those in Figure~\ref{fig:ipvs_l3dsr_10g} by placing them in node net namespace.
    }
...
While the CPU usage of the \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} becomes less than that of iptables DNAT, the CPU usage of the ipvs is still larger than that of iptables DNAT.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[CPU usage of load balancers on nodes]{
      ...
      CPU usages of ipvs and \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} greatly improved from those in Figure~\ref{fig:cpu_usage_10g} by placing them in node net namespace.  
      While the \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} consumes the smallest amount of the CPU resource, the CPU usage of ipvs is still larger than that of iptables DNAT.
    }
...
Although the proposed load balancer may not be the most efficient one, the author shows that it has \replaced[id=2nd]{sufficient}{enough} throughput for 10Gbps network environment.
...
The maximum throughput in 1Gbps, i.e., 293K [req/seq] is \replaced[id=2nd]{already}{easily} achieved by single \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} (293K [req/seq]) or two of ipvs load balancers (193K x 2 [req/seq]) with ECMP redundancy.
...
\replaced[id=2nd]{However, t}{T}his is still \deleted[id=2nd]{easily} achievable \added[id=2nd]{without much of the hassle} by using four of \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} (731K x 4 [req/seq]) or nine of ipvs load balancers (335K x 9 [req/seq]) with ECMP redundancy.
...
This can be \deleted[id=2nd]{easily }achievable using four of the \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} (L3DSR) load balancer container since a single \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} in a container can handle 731K [req/sec].
Therefore the author considers the proposed load balancer has \replaced[id=2nd]{sufficient}{enough} performance for 10 Gbps network environment.
