...
\added[id=3rd]{
  ...
  The author has found that the choice of the operation mode greatly affects the throughput of the load balancers, as will be explained in Chapter~\ref{chapter:Performance Evaluation}.
  Therefore, flannel, which is one of the overlay networks and is used in this study, is explained in detail.
}

\replaced[id=3rd]{Secondly,}{Then} the author explains how to utilize multi-core CPUs for packet processing in Linux\replaced[id=3rd]{.}{,}
\added[id=3rd]{
The Linux kernel comes with built-in features for multi-core packet processing.
However, the default setting of Linux distribution might not necessarily be optimized for best performance and sometimes hinder the understanding of experimental results.
That was the case when the author first carried out the experiment.
}

\added[id=3rd]{
  Thirdly, the author explains Linux kernel's IPVS load balancer, because IPVS is used to implement a load balancer in a container.
  The IPVS has three operating modes and they are also explained.
}
...
\replaced[id=3rd]{Lastly,}{Finally,} the author briefly explains about novel XDP technology.
%
\added[id=3rd]{
The author plans to replace IPVS based load balancer with the one based on XDP technology in future work.
The author has already started the implementation of such load balancer and presented a preliminary result in Chapter~\ref{chapter:Further Improvement}.
...
\added[id=3rd]{
  ...
  In the case of Kubernetes, every {\em pod}}
\footnote{\added[id=3rd]{A {\em pod} is a group of containers that share the same network namespace and cgroup, and is the basic execution unit in a Kubernetes cluster.}}
\added[id=3rd]{
 is assigned with its own IP address.
...
The kernel normally does not know this, but \replaced[id=2nd]{if there is}{with} the help of overlay network \cite{zismer2016performance}, the kernel eventually finds out the next hop towards peer container.
...
\begin{table}
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[Flannel backend modes]{
      ...
      \deleted[id=3rd]{Viable flannel backend modes. In cloud environment tunneling using vxlan or udp is needed.}
    }
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[A network architecture of container cluster system]{
      \added[id=3rd]{A network architecture of container cluster system.}
      \deleted[id=3rd]{The network architecture of an exemplified container cluster system.}
