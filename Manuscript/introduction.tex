...
A group of these servers is often called a web cluster or a web application cluster.
...
There are also a set of load balancers that distribute requests from clients to multiple web servers. 
Web servers may store some important data into DB servers and retrieve those data from DB servers as needed.
...
From the point of view of cloud users (i.e. web application providers), this lowers initial cost spent on infrastructures for their services.
It also shortens the time to start a service from the inception of the project, hence gives the users agility.
And at the time when the computing resources for a service is excessive, users can easily decrease the VMs and reduce the cost.
