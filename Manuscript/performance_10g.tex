...
Using containr network, i.e. veth+bridge, further degrade the throughput of ipvs.

\FloatBarrier

\section{Required throughput for the proposed load balancer}

The author has compared the performance of proposed load balancers in 10Gbps.
ssAlthough the proposed load balancer may not be the most efficient one, it is still usefull because of the lateral scalability.
...
For example, the bottleneck at the benchmark client is about 293K [req/sec] in the 1Gbps network, the achievable load balancers should handle at most 293K [req/sec] equivalent traffic.
...
In the case of 10Gbps network, the muximum throughput determined by the bottleneck at the entrance is 2.9M [req/sec].
This is still easily achievable by using four ipvs-tun (731K x 4 [req/seq]) or nine of ipvs load balancers (335K x 9 [req/seq]) with ECMP redundancy.
...
Although single core is used for the packet processing, the throughput of the xlb load balancer is close to half of the iptables DNAT's with 16 core(eight physical corex) packet processing.
