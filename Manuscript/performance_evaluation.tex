...
\added[id=2nd]{
In this chapter, the author evaluates the feasibility of the proposed load balancer.
...
%
In this specific experiment, \added[id=2nd]{it was found that} the performance level\replaced[id=2nd]{s of IPVS and iptables DNAT were}{ was} limited by the 1 Gbps \replaced[id=2nd]{network bandwidth at the load balancer node.}{ bandwidth of the network,}\footnote{All of the nodes use a single interface for communication. 
...
\deleted[id=2nd]{and therefore, we can not see the real difference between ipvs performance and iptables DNAT performance.}
\added{This \replaced[id=2nd]{was}{fact has been}}\deleted{which is} revealed by packet\added[id=2nd]{-}level analysis using tcpdump (Appendix~\ref{appendix:performance_limit}).
%
...
When the \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} send out the packets to real servers, it encapsulates the original packet in ipip tunneling packet \cite{kuznetsov1999tunnels} that is destined to real servers.
...
As can be seen in the figure, while the performance levels for \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT exactly match\added{ because of the network \replaced[id=2nd]{bandwidth}{band width} limitation}, the performance level of \replaced[id=2nd]{IPVS-TUN}{ipvs-tun} is much higher than those.
...
\added{
  ...
  In the case of \replaced[id=2nd]{IPVS-TUN, it turned out that the maximum throughput is limited by the bandwidth at the benchmark client}{ipvs-tun, the response packet could fill the network \replaced[id=2nd]{bandwidth}{band width}s at the benchmark client}.
  Multipling the maxium throughput of about 290K [req/sec] with the response packet size 450[byte/req] (Appendix~\ref{appendix:performance_limit}) and 8 [bit/byte] results in 972M [bit/sec] .
  \replaced[id=2nd]{
    The fact that the load balancer itself is not the performance limiting factor means that proposed load balancer has sufficient performance levels in 1 Gbps network.
...
\added[id=2nd]{
In this chapter, the author evaluated the feasibility of the proposed load balancer.
