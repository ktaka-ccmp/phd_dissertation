...
If a web application is portable, it will become much easier for web application providers to migrate the service when there is a disaster, or expand their business to different geographical locations, etc. 
}

\deleted{As web applications become an essential part of daily life,} \replaced[id=2nd]{Nowadays,}{These days,} an outage of the web application service \replaced[id=2nd]{has already become a critical}{is getting to be a bigger} problem \cite{}.
...
Because there are many flavors of Linux distributions, and even if the same distribution is used, there was always a chance that a slight difference in a program binary version or library versions could \replaced[id=2nd]{break}{have broken} the expected behavior.
...
In conventional technologies, processes on a single server are dependent on common shared libraries, and hence updates of the library sometimes have caused unexpected side effects\added[id=2nd]{to those programs that use the library}.
...
A container orchestrator is a tool to simplify the management of a cluster of containers that are launched on multiple servers.
%
...
Users only need to feed the configuration file to the orchestrator to launch \replaced[id=2nd]{this}{the} web application.
...
Although all of these orchestrators mostly satisfy the requirements, they \added{fail to support the automatic routing of the ingress traffic.}
\added{Docker swarm and Mesos/Marathon} rely on manual set up of a static routing for ingress traffic.
\added{Only }Kubernetes has the functionality to manage cloud load balancer so that ingress traffic from the Internet is \added{automatically} routed to containers in a redundant and scalable manner\added{.
  However,}
...
In this way, Kubernetes fails to provide uniformity\deleted{ that is essential for easy deployment and migration of complex}\added{ to improve the portability of} web applications.
...
\added{
  ...
  To bring this into reality, container orchestrators need to function as a standard middleware. Container orchestrators must provide the same interfaces to web applications, regardless of the base infrastructure, e.g., cloud providers or on-premise data centers.
...
\added{
  ...
  Proposed load balancer is runnable in both cloud infrastructure and on-premise data center and can be utilized to set up the route for ingress traffic automatically.
  The proposed load balancer should have the following features;
}
%
...
\added{
While the method to acquire information about running containers is specific to Kubernetes, other technologies used are not.
Therefore, the author expects the proposed load balancer can also be implemented for other container orchestrators, i.e., Docker Swarm and Mesos/Marathon, just by writing the code to acquire information about running containers.
}
...
3) Performance: whether the load balancer has enough throughput for 10 Gbps network.
}
...
\added{The author carries out a performance evaluation and quantitably prove the feasibility of the proposed load balancer up to 10Gbps network environment.}
...
The outcome of this study will\deleted{ benefit users who want to deploy their web applications on any cloud provider where no scalable load balancer is provided, to achieve high scalability.
