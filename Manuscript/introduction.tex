...
\centering\parbox[c]{0.9\columnwidth}{
Example web clusters consisting of a cluster of containers are deployed in three different data centers.
In each of the data center, container orchestrator manages a web cluster. Important data are stored in a globally consistent data store.  
Access from the client is routed to the closest data center using anycast technology.
}
\label{fig:global_container_infrastructure}
\end{figure}

In order to realize an easy migration of web applications, an ideal infrastructure should have the following features;
...
Figure\ref{fig:global_container_infrastructure} shows an exemplified infrastructure having these features, as a universal middleware.
...
Orchestrator must also maintain the state described in configuration files, for example, the orchestrator must maintain the number of running containers.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|c|c|}
    ...
    Config file & Yaml & Yaml & unknown  \\ \hline
  ...
  \raggedright
  aaaa 1) Multi-layer service description in configuration files
  \label{table:orchestrator_comparison}
\end{table}

Table\ref{table:orchestrator_comparison} compares important features for these orchestrators, as a web application base infrastructure.
All of these orchestrators rely on static routing for ingress traffic.
Only Kubernetes has the functionality to manage cloud load balancer so that ingress traffic from the Internet is routed in a redundant and scalable manner.
However, this functionality is available only in a few cloud environments.
...
The purpose of this study is to seek a generic way to route the traffic into container clusters automatically, by providing a software load balancer that works well with the container management systems, and thereby to facilitate web application migrations.
Figure~\ref{fig:cluster_of_container_loadbalancer} shows ...

Since Kubernetes is most appropriate for web application clusters at the moment, the author investigates portable software load balancer for Kubernetes as a test case.
However the author expect general findings can be easily applied to the other container orchestrators.
