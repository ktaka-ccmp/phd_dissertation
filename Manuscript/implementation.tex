...
%
Each load balancer pod consists of an exabgp \cite{exa-networks_2018} container and an \replaced[id=2nd]{IPVS}{ipvs} container.
The \replaced[id=2nd]{IPVS}{ipvs} container is responsible for distributing the traffic toward the service IP to web server (nginx) pods.
...
The \replaced[id=2nd]{IPVS}{ipvs} container monitors the availability of web server pods by consulting apiserver on the master node and manages the load balancing rule appropriately.
...
The author also used conventional Linux system as an upstream router for testing purpose, using the same OS as the nodes and \added[id=2nd]{the} route reflector.
...
Although in the actual production environment\deleted[id=2nd]{,} proprietary hardware router with the highest throughput is usually deployed, one can still test some of the advanced features by using a conventional Linux system \replaced[id=2nd]{for the upstream}{as the} router.

\begin{table}[h]
  \centering
  \begin{tabular}{|l|c|c|c|}
    \hline
    \begin{tabular}{l} \added[id=2nd]{Features} \end{tabular} & \multicolumn{1}{c|}{gobgpd} & \multicolumn{1}{c|}{exabgp} & \multicolumn{1}{c|}{bird} \\ \hline
...
%    \begin{tabular}{l} Use case \end{tabular} & Router/Route reflector & Load balancer & Not used$^{**}$  \\ \hline
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[Comparison of open source BGP agents]{
      ...
      $^{**}$ \phantom{$^{}$} \deleted[id=2nd]{Configuration of bird was more complex than other agents. }\added[id=2nd]{The dynamic-neighbor is the feature to describe peer group as a range of IP address.}
    }
...
Each load balancer {\em pod} needs to advertise \replaced[id=2nd]{itself as the next hop for packets toward the service IP.}{a static route for the service IP to itself. }
...
\replaced[id=2nd]{On the other hand, the}{ The} gobgpd and the bird require other client programs to inject the static route after the daemon itself is up and running.
...
This is a bit complex and error-prone, especially when one \replaced[id=2nd]{has to inject the route periodically}{needs to make sure the route injection is done} inside a container.

As for the route reflector, add-path \cite{rfc7911} feature \replaced[id=2nd]{and dynamic neighbor feature are needed.}{is needed for multi-path advertisement, } \replaced[id=2nd]{These are}{which is} supported by gobgpd.
...
\deleted[id=2nd]{The other features needed for the route reflector are a dynamic-neighbor feature to describe peer group as a range of IP address, and overlay network feature on the Linux system.}
\added[id=2nd]{The other requirement for the route reflector is the knowledge of the overlay network for the containers.}
...
\subsection{\replaced[id=2nd]{IPVS}{Ipvs} container}\label{sec:ipvs}

In order to \replaced[id=2nd]{implement}{demonstrate} a software load balancer that is runnable in any environment, the author \replaced[id=2nd]{used Linux kernel's IPVS in Docker container.}{ created Docker container using ipvs.}
In addition to that, the proposed load balancer uses two other components, keepalived\deleted[id=2nd]{ and the }{, and a }controller.
These components are placed in a single Docker container image.
\added[id=2nd]{The Dockerfile, a shell script file and the template files used to create the load balancer container are shown in Appendix~\ref{appendix:ipvs_container}. }
The \replaced[id=2nd]{IPVS}{ipvs} is a Layer-4 load balancer capability, which is included in the Linux kernel 2.6.0 released in 2003 or later,
...
For example, \replaced[id=2nd]{IPVS}{ipvs} distributes incoming Hypertext Transfer Protocol (HTTP) traffic destined for a single destination IP address,
...
and manages \replaced[id=2nd]{IPVS}{ipvs} balancing rules in the kernel accordingly.
It is often used together with \replaced[id=2nd]{IPVS}{ipvs} to facilitate ease of use.
...
The proposed load balancer needs to dynamically reconfigure the \replaced[id=2nd]{IPVS}{ipvs} balancing rules whenever {\em pods} are created or deleted. 
Figure~\ref{fig:ipvs-ingress-schem} is a schematic diagram of \replaced[id=2nd]{IPVS}{ipvs} container to show the dynamic reconfiguration of the \replaced[id=2nd]{IPVS}{ipvs} rules.
Two daemon programs, controller and keepalived, running in the container are illustrated.
The keepalived manages Linux kernel's \replaced[id=2nd]{IPVS}{ipvs} rules depending on the ipvs.conf configuration file.
...
If the health check to a {\em real server} fails, keepalived will remove that {\em real server} from the \replaced[id=2nd]{IPVS}{ipvs} rules immediately.
  ...
  \includegraphics[width=0.8\columnwidth]{Figs/ipvs-ingress-schem}
    ...
    \begin{minipage}{0.9\columnwidth}
      \caption[Implementation of IPVS container]{
        Implementation of \replaced[id=2nd]{IPVS}{ipvs} container.
...
Then, keepalived will reload the ipvs.conf, and modify \replaced[id=2nd]{the IPVS rules inside the kernel appropriately}{ the kernel's ipvs rules correctly} depending on the result of the health check.

When a pod is terminated, existing connections are reset by the node kernel.
The SYN packets sent to a pod after termination, but before the \replaced[id=2nd]{IPVS}{ipvs} rule update, will be answered with ICMP unreachable by the node.
In these cases, the client sees connection errors.
\deleted[id=2nd]{In order to avoid the connection errors to be seen by a human, HTTP client programs are required to re-initiate the connection.}
However, since the load balancer rule update is within a second, these errors can be regarded as the tolerable rare exceptions\deleted[id=2nd]{ even without such re-initiations}.
\added[id=2nd]{In order to avoid the connection errors to be seen by a human, HTTP client programs are required to re-initiate the connection.}
...
%
Keepalived and the controller are placed in the docker image of \replaced[id=2nd]{IPVS}{ipvs} container.
The \deleted[id=2nd]{ipvs is the kernel function and }namespace separation for \replaced[id=2nd]{IPVS}{container} has already been supported in the recent Linux kernel. 

Configurations for capabilities were needed when deploying the \replaced[id=2nd]{IPVS}{ipvs} container: adding the CAP\_SYS\_MODULE capability 
to the container to allow the kernel to load required kernel modules inside a container, 
and adding CAP\_NET\_ADMIN capability to the container to allow keepalived to manipulate the kernel's \replaced[id=2nd]{IPVS}{ipvs} rules. 
...
\begin{figure}[tb]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[Network path by the exabgp container]{
      ...
      And then the \replaced[id=2nd]{IPVS}{ipvs} container forwards them to nginx pods (red arrows).
...
\added[id=2nd]{The Dockerfile and a shell script file used to create the BGP software container are shown in Appendix~\ref{appendix:exabgp_container}. }
...
After that the \replaced[id=2nd]{IPVS}{ipvs} forwards them to nginx pods.
The IP address of any pod is dynamically assigned from 172.16.0.0/16 when the pod is started. 

Table~\ref{table:exabgp_setting} summarises some key settings required in the exabgp container to route the traffic to the \replaced[id=2nd]{IPVS}{ipvs} container.
(1) In BGP announcements the node IP address, 10.0.0.106 is used as the next-hop for the service IPs, 10.1.1.0/24.
(2) Then on the node, in order to route the packets toward the service IPs to the \replaced[id=2nd]{IPVS}{ipvs} container, 
