...
The performance levels of \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT have been limited by 1 Gbps bandwidth.
...
The \replaced[id=2nd]{IPVS}{ipvs} has a mode called ipvs-tun.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.9\columnwidth}
    \caption[Throughput of L3DSR using ipvs-tun.]{
      ...
      The throughput of ipvs-tun is 1.5 times higher than those of conventional \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT.
    }
...
Figure~\ref{fig:ipvs_l3dsr_1g.png} compares the throughput of the ipvs-tun, conventional \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT.
As can be seen in the figure, while the performance levels for \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT exactly match\added{ because of the network band width limitation}, the performance level of ipvs-tun is much higher than those.
For example, the throughput of ipvs-tun is about 1.5 times higher than those of conventional \replaced[id=2nd]{IPVS}{ipvs} and iptables DNAT.
\added{
  The saturated throughput indicates the maximum performance level of the load balancer, which is determined either by network bandwidth, or CPU performance levels of these machines.
  In the case of ipvs-tun, the response packet could fill the network band withs at the benchmark client.
...
%% \footnote{\deleted{
%% The limit is due to the bandwidth of benchmark client's NIC.
...
3) The NIC of the benchmark client has been changed to 10 Gbps card since now we have multiple of \replaced[id=2nd]{IPVS}{ipvs} container load balancers that are capable of filling up 1 Gbps bandwidth.
...
The performance level increases linearly as the number of the load balancers increases up to four of the \replaced[id=2nd]{IPVS}{ipvs} load balancers, but does not scale further.
...
%
(3) The throughput of the \replaced[id=2nd]{IPVS}{ipvs} in a container is equivalent to that of the iptables DNAT as a load balancer, in 1Gbps network environment.
(4) The throughput of the \replaced[id=2nd]{IPVS}{ipvs} container can be further improved up to 1.5 times by utilizing ipvs-tun mode, i.e., L3DSR settings.
(5) The \replaced[id=2nd]{IPVS}{ipvs} container is able to run and function properly, both in GCP and AWS.
