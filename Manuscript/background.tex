...
1) There must be cloud load balancers whose APIs are supported by the Kubernetes daemons.
...
Regarding the first proble, there are numerous load balancers which is not supported by the Kubernetes.
...
As for the second problem, imagine a situation in which the DNAT table on one of the nodes malfunctions.
