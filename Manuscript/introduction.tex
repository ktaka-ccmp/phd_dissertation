...
\subsection{\replaced{Portability}{ Migration } of web application cluster}
...
Doing so will give users freedoms to migrate when there is a disaster, expand their business, and prevent vendor lock-ins, etc. 
In order to do this, container orchestrators need to function as a standard middleware that provides the same interfaces to web applications, regardless of the base infrastructure, e.g., cloud providers or on-premise data centers.
However, existing orchestrators fail to function as a standard middleware, since the way to route the ingress traffic is either by setting up a static route or by relying on cloud load balancers.
}
