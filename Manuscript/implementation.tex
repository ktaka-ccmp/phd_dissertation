...
Although in the actual production environment, proprietary hardware router with the highest throughput is usually deployed, one can still test some of the advanced functions by using a Linux box as the router.
...
\begin{table}[H]
  ...
  \centering\parbox[c]{0.9\columnwidth}{
    ...
    \small
    $^{*}$ \phantom{$^{**}$} Injector is commandline or other programs other than the BGP daemon. \par
    $^{**}$ \phantom{$^{*}$} Add-path is the feature to support multipath advertisement. \par
    $^{***}$ \phantom{$^{}$} Configuration of bird was complex.
  }
