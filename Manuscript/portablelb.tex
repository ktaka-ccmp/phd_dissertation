This chapter discusses portability and performance level of a single ipvs load balancer.
...
The performance levels of ipvs and iptables DNAT have been limited by 1 Gbps band width.
...
Sice the souce IP address of original packet is maintained, the returning packets are sent directly toward the benchmark client.
In this scheme the returning packets do not consume the band width nor the CPU power of the load balancer node.
...
Figure~\ref{fig:ipvs_l3dsr_1g.png} shows the throughput of the ipvs-tun, ipvs-nat and iptables DNAT.
As can be seen in the figure, the performance levels is greatly improved ipvs-tun.
...
Both results show similar characteristics as the experiment in an on-premise data center in Figure~\ref{fig:ipvs_mcore_proccessing}, where throughput increases linearly to a certain saturation level that is determined by either network speed or machine specifications.
It is shown that the proposed load balancer can be run in cloud environments and also functions properly.
...
This may be because the physical servers in  on-premise data center outperform the VMs in a cloud environment, or because network bandwidth is limited based on the type of instances.
...
Still, we can say that the proposed ipvs load balancers can be run in both GCP and AWS, and the behavior is the same with the load balancers in on-premise data centers.

\FloatBarrier

\section{Summary}

In this chapter portability and performance level of proposed load balancer are discussed.
